(function (global, factory) {
  ...
  */
  ...
  var Entity = function () {
    ...
    _createClass(Entity, [{
      key: "init",
      value: function init(entityData) {
      ...
      value: function refresh() {
        ...
        return new Promise(function (resolve, reject) {
          _Nymph.Nymph.getEntityData({ "class": _this.constructor.class }, { "type": "&", "guid": _this.guid }).then(function (data) {
      ...
      value: function referenceSleep(reference) {
        ...
        this.sleepingReference = [].concat(_toConsumableArray(reference));
      }
    }, {
      key: "ready",
      ...
      key: "readyAll",
      value: function readyAll(success, error, level) {
        var _this4 = this;
              ...
              for (var k in _this4.data) {
                if (_this4.data.hasOwnProperty(k)) {
                  if (_this4.data[k] instanceof Entity) {
                    promises.push(_this4.data[k].readyAll(undefined, undefined, newLevel));
                  } else if (isArray(_this4.data[k])) {
                    for (var i = 0; i < _this4.data[k].length; i++) {
                      if (_this4.data[k][i] instanceof Entity) {
                        promises.push(_this4.data[k][i].readyAll(undefined, undefined, newLevel));
                      }
                    }
                  }
                }
  ...
  _Nymph.Nymph.setEntityClass(Entity.class, Entity);

  // === Error Classes ===

  var EntityIsSleepingReferenceError = function (_extendableBuiltin2) {
    _inherits(EntityIsSleepingReferenceError, _extendableBuiltin2);
