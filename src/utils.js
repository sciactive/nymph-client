...
export function getDataReference (item) {
  if (item instanceof Nymph.getEntityClass('Nymph\\Entity') && typeof item.toReference === 'function') {
...
export function getSleepingReference (item) {
  ...
  } else if (item instanceof Object && !(item instanceof Nymph.getEntityClass('Nymph\\Entity'))) {
    for (let k in item) {
      if (item.hasOwnProperty(k)) {
...
export function sortObj (obj) { // adapted from http://am.aurlien.net/post/1221493460/sorting-javascript-objects
  const tempArray = [];
  for (let key in obj) {
